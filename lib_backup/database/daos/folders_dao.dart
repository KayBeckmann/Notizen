...
@DriftAccessor(tables: [Folders, Notes, NoteTags])
class FoldersDao extends DatabaseAccessor<AppDatabase> with _$FoldersDaoMixin {
  ...
  Future<void> deleteFolder(String id) async {
    ...
    // Alle Notizen in diesem Ordner finden
    final folderNotes =
        await (select(notes)..where((t) => t.folderId.equals(id))).get();
    final noteIds = folderNotes.map((n) => n.id).toList();

    if (noteIds.isNotEmpty) {
      // Tags für alle Notizen in diesem Ordner löschen
      await (delete(noteTags)..where((t) => t.noteId.isIn(noteIds))).go();
      // Notizen selbst löschen
      await (delete(notes)..where((t) => t.folderId.equals(id))).go();
    }
